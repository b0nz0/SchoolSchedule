...
def school_select_screen():
    ...
    # school selection
    school_label = ttk.Label(school_select_frame, text=SCHOOL_SELECT_LABEL)
    ...
    school_add_button = ttk.Button(school_select_frame, text=ADD_SCHOOL_LABEL, command=gui.event.school_add)
    school_add_button.grid(column=11, row=0, sticky=(N, E, W, S))
    school_add_button.state(['!disabled'])

    school_delete_button = ttk.Button(school_select_frame, text=DELETE_SCHOOL_LABEL, command=gui.event.school_delete)
    school_delete_button.grid(column=12, row=0, sticky=(N, E, W, S))
    school_delete_button.state(['disabled'])

    # school year selection
    schoolyears_label = ttk.Label(school_select_frame, text=SCHOOLYEAR_SELECT_LABEL)
    schoolyears_label.grid(column=0, row=1, sticky=(N, E, W, S))

    schoolyearssvar = StringVar()
    schoolyears_combo = ttk.Combobox(school_select_frame, textvariable=schoolyearssvar)
    ...
    schoolyear_add_button = ttk.Button(school_select_frame, text=ADD_SCHOOLYEAR_LABEL, command=gui.event.schoolyear_add)
    schoolyear_add_button.grid(column=11, row=1, sticky=(N, E, W, S))
    schoolyear_add_button.state(['!disabled'])

    schoolyear_delete_button = ttk.Button(school_select_frame, text=DELETE_SCHOOLYEAR_LABEL, command=gui.event.schoolyear_delete)
    schoolyear_delete_button.grid(column=12, row=1, sticky=(N, E, W, S))
    schoolyear_delete_button.state(['disabled'])

    #year selection
    year_label = ttk.Label(school_select_frame, text=YEAR_SELECT_LABEL)
    year_label.grid(column=0, row=2, sticky=(N, E, W, S))

    years_listbox = ttk.Treeview(school_select_frame, show="tree")
    scrollbar = ttk.Scrollbar(school_select_frame, orient=VERTICAL, command=years_listbox.yview)
    ...
    year_add_button = ttk.Button(school_select_frame, text=ADD_YEAR_LABEL, command=gui.event.year_add)
    year_add_button.grid(column=11, row=2, sticky=(E, W))
    year_add_button.state(['!disabled'])

    year_delete_button = ttk.Button(school_select_frame, text=DELETE_YEAR_LABEL, command=gui.event.year_delete)
    ...
    school_select_frame.rowconfigure(0, minsize=height)
    school_select_frame.rowconfigure(1, minsize=height)
    school_select_frame.columnconfigure(0, minsize=width)
    school_select_frame.columnconfigure(1, minsize=width)
